...
async function handle_request(msg, callback) {
	...
	switch (msg.api) {
						...
						if (error) {
							...
							for (var i = 0; i < data.length; i++) {
								...
								for (var j = 0; j < data[i].menu.length; j++) {
									...
									if (
										data[i].menu[j].dishname.toLowerCase() ===
										keyword.toLowerCase()
									) {
		...
		case "fetch_userp": {
			...
			try {
				const user = await Users.findOne({ _id: message.userId }, function (
					error,
					data
				) {
					if (error) {
		...
		case "fetch_event": {
			...
			const eventdata = [];
			try {
				await Events.find({ restaurantId: message.restaurantId }, function (
					error,
					data
				) {
					if (error) {
